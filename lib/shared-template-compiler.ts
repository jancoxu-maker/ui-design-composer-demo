...
function cleanPage(page?: Partial<PageIR>): PageIR {
  const source = page || {};
  const stats = Array.isArray(source.stats) && source.stats.length ? source.stats.slice(0, 4) : defaultPageIR.stats;
...
function editorialMarkup(page: PageIR) {
  ...
  return `<div class="editorial-shell"><header class="masthead" data-template-role="masthead"><b>${escapeHtml(page.brand)} REVIEW</b><span>ISSUE 04 · 2026</span><a data-template-role="primary-action" href="#story">${escapeHtml(page.primaryAction)} ↗</a></header><main><section class="hero" data-template-role="hero"><small>${escapeHtml(page.eyebrow)}</small><h1>${escapeHtml(page.title)}</h1><p>${escapeHtml(page.description)}</p></section><section class="editorial-stage" id="story"><article class="cover"><span>01</span><div class="cover-art" aria-hidden="true"><i></i><i></i></div><footer><b>${escapeHtml(lead?.title || page.title)}</b><small>${escapeHtml(lead?.meta || '')}</small></footer></article><article class="story" data-template-role="story"><small>FIELD NOTE · 014</small><h2>${escapeHtml(page.secondaryAction)}</h2><p>${escapeHtml(lead?.description || page.description)}</p><ul>${page.items.slice(1, 5).map((item) => `<li><span>${escapeHtml(item.status)}</span><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.meta)}</small></li>`).join('')}</ul></article></section></main></div>`;
}
...
function precisionMarkup(page: PageIR) {
  return `<main class="hardware-shell" data-template-role="console"><header class="hardware-nav" data-template-role="navigation"><b>${escapeHtml(page.brand)} / 84</b><span>MASTER CONTROL · READY</span><a data-template-role="primary-action" href="#controls">${escapeHtml(page.primaryAction)}</a></header><section class="hero" data-template-role="hero"><small>${escapeHtml(page.eyebrow)}</small><h1>${escapeHtml(page.title)}</h1><p>${escapeHtml(page.description)}</p></section><section class="hardware-panel" id="controls" data-template-role="controls"><article class="meter"><header><b>${escapeHtml(page.items[0]?.title || 'MASTER OUTPUT')}</b><span>${escapeHtml(page.items[0]?.status || 'READY')}</span></header><div class="vu"><i></i><i></i><strong>${escapeHtml(page.stats[0]?.value || '72')}</strong></div><footer>${escapeHtml(page.items[0]?.description || page.description)}</footer></article><article class="control-bank">${page.stats.slice(0, 3).map((item, index) => `<div class="control"><i class="dial dial-${index + 1}" aria-hidden="true"></i><b>${escapeHtml(item.label)}</b><small>${escapeHtml(item.detail)}</small></div>`).join('')}</article></section></main>`;
}
...
function kineticMarkup(page: PageIR) {
  ...
  return `<div class="kinetic-shell">${chrome(page)}<main><section class="hero" data-template-role="hero"><small>${escapeHtml(page.eyebrow)}</small><h1>${lines.map((line) => `<span>${escapeHtml(line)}</span>`).join('')}</h1><p>${escapeHtml(page.description)}</p><a class="hero-action" href="#work">${escapeHtml(page.primaryAction)} <i aria-hidden="true"></i></a></section><div class="kinetic-orbit" aria-hidden="true"><i></i><i></i><b>01</b></div><section class="kinetic-stage" id="work" data-template-role="stage">${page.items.slice(0, 3).map((item, index) => `<article><span>0${index + 1}</span><h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.description)}</p></article>`).join('')}</section></main><footer><span>${escapeHtml(page.brand)} / WORLDWIDE</span><span>${escapeHtml(page.secondaryAction)}</span><span>©26</span></footer></div>`;
}
