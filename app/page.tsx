...
import type { DeliveryScaleMode } from './components/responsive-delivery-preview';
...
type DesignVariant = { id: 'safe' | 'balanced' | 'bold'; title: string; summary: string; html: string; coverage: DesignCoverage; templateId: string; templateFamily: string; templateVerified: boolean; templateParity?: TemplateParity };
...
export default function Home() {
  ...
  const liveTemplateSelections = useMemo(() => ({ goal, audience, uxPattern, visualDirection, style, template, typeTone, colorStrategy, corners, depth, imageStyle, componentTone, motionProfile, directionSettings: activeDirectionSettings, density, motion, preserve, locks, accent }), [goal, audience, uxPattern, visualDirection, style, template, typeTone, colorStrategy, corners, depth, imageStyle, componentTone, motionProfile, activeDirectionSettings, density, motion, preserve, locks, accent]);
    ...
    <main className="app" style={{ '--accent': accent, '--accent-contrast': accentContrast(accent) } as CSSProperties}>
      ...
      <div className="layout" id="top">
        ...
        <section className={`controls step-${step}`} aria-live="polite">
          ...
          <div className="control-body">
            {step === 1 && <div className="source-flow">
              ...
              {source === 'url' && <div className={`url-box state-${urlState}`}><label htmlFor="page-url">公开页面参考地址</label><div><input id="page-url" type="url" value={pageUrl} onChange={(event) => { setPageUrl(event.target.value); setUrlState('idle'); }} onKeyDown={(event) => { if (event.key === 'Enter') recognizeUrl(); }} placeholder="https://your-product.com/dashboard" aria-invalid={urlState === 'error'}/><button type="button" onClick={recognizeUrl}>{urlState === 'ready' ? '更新地址' : '加入材料'}</button></div><p>{urlState === 'error' ? '请输入可公开访问的网址，例如 https://example.com。' : urlState === 'ready' ? '生成时会安全读取公开可见文字；登录页或脚本页面请改用截图/HTML。' : '仅读取单个公开网页，不访问登录内容、本机或局域网地址。'}</p></div>}
        ...
        <section className="preview" aria-label="视觉引擎与生成结果">
          <header><div><i/> <b>{activeVariant ? 'HTML 交付结果' : '视觉引擎演示'}</b><small>{activeVariant ? activeVariant.title : `${activeRecipe.engine} · 非原稿预览`}</small></div>{activeVariant ? <nav className="output-tabs" aria-label="HTML 结果查看方式"><button className={outputMode === 'preview' ? 'active' : ''} type="button" onClick={() => setOutputMode('preview')}>效果</button><button className={outputMode === 'code' ? 'active' : ''} type="button" onClick={() => setOutputMode('code')}>代码</button><button className={outputMode === 'agent' ? 'active' : ''} type="button" onClick={() => setOutputMode('agent')}>智能体应用包</button><button type="button" onClick={copyCurrentOutput}>{copyState !== 'idle' ? '已复制' : outputMode === 'agent' ? '复制应用包' : '复制 HTML'}</button><button type="button" onClick={downloadHtml}>下载 HTML</button></nav> : <nav aria-label="Web 演示缩放"><span className="preview-device-label">WEB</span><button className={previewZoom === 85 ? 'active' : ''} type="button" onClick={() => setPreviewZoom(previewZoom === 100 ? 85 : 100)}>{previewZoom}%</button></nav>}</header>
          {activeVariant && outputMode === 'preview' && <div className="delivery-toolbar"><span className="delivery-web-label">WEB · 1440 × 900</span><label className="delivery-type-control"><span><b>标题大小</b><output>{headingScale}%</output></span><input aria-label="调整各级标题大小" type="range" min="80" max="125" step="5" value={headingScale} onChange={(event) => setHeadingScale(Number(event.target.value))}/></label><nav aria-label="交付预览缩放">{([['fit','适应窗口'],['actual','100%']] as const).map(([id,label]) => <button key={id} type="button" className={deliveryScaleMode === id ? 'active' : ''} onClick={() => setDeliveryScaleMode(id)}>{label}</button>)}</nav></div>}
          <div className={`preview-shell ${activeVariant && outputMode === 'preview' ? 'delivery-result-shell' : ''}`}><div className={`preview-viewport ${activeVariant ? 'delivery-mode' : `device-desktop zoom-${previewZoom}`}`}>{activeVariant ? outputMode === 'preview' ? <ResponsiveDeliveryPreview html={deliveryHtml} title={activeVariant.title} device="desktop" scaleMode={deliveryScaleMode}/> : outputMode === 'agent' && agentTemplatePackage ? <div className="agent-delivery"><header><span><b>跨智能体模板应用包</b><small>包含关键词、设计约束、当前配置与可复用核心代码</small></span><em>{agentTemplatePackage.prompt.length.toLocaleString()} 字符</em></header><section><b>模板关键词</b><div>{agentTemplatePackage.keywords.map((keyword) => <span key={keyword}>{keyword}</span>)}</div></section><pre><code>{agentTemplatePackage.prompt}</code></pre></div> : <div className="code-delivery"><header><span><b>完整单页 HTML</b><small>{activeVariant.templateId} {isSharedTemplate(activeVariant.templateId) ? '共享模板已编译' : '视觉模板已编译'} · 标题 {headingScale}% · 包含结构、样式、响应式与动效</small></span><em>{deliveryHtml.length.toLocaleString()} 字符</em></header><pre><code>{deliveryHtml}</code></pre></div> : <TemplateDocumentPreview direction={visualDirection} title={previewTitle} selections={liveTemplateSelections} device="desktop" variant="balanced"/>}</div></div>
